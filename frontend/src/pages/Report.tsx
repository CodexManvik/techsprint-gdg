...
export const Report = () => {
  ...
  const transformReportData = (backendData: any): ReportData => {
    ...
    if (backendData.analytics) {
      ...
      const aiReport = backendData.ai_report;
      
      // Use AI-generated scores if available, otherwise fall back to analytics
      const radarChart = aiReport?.radar_chart;
      
      return {
        summary: aiReport?.summary || analytics.summary || "Interview completed successfully.",
        radarData: [
          { 
            category: 'Technical', 
            user: radarChart?.technical_accuracy || analytics.technical_score || 75, 
            ideal: 90 
          },
          { 
            category: 'Communication', 
            user: radarChart?.communication_clarity || analytics.communication_score || 70, 
            ideal: 85 
          },
          { 
            category: 'Confidence', 
            user: radarChart?.confidence_level || analytics.confidence_score || 75, 
            ideal: 88 
          },
          { 
            category: 'Body Language', 
            user: radarChart?.cultural_fit || (analytics.posture_avg * 100) || 70, 
            ideal: 85 
          },
          { 
            category: 'Problem Solving', 
            user: radarChart?.problem_solving || analytics.problem_solving || 80, 
            ideal: 90 
          },
        ],
        metrics: [
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <span className={`px-4 py-2 rounded-lg font-bold text-lg ${
                      reportData.aiFeedback.hiring_verdict === 'STRONG HIRE' ? 'bg-green-100 text-green-800' :
                      reportData.aiFeedback.hiring_verdict === 'HIRE' ? 'bg-blue-100 text-blue-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                      {reportData.aiFeedback.hiring_verdict}
                    </span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {reportData.aiFeedback.strengths.map((strength, idx) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {reportData.aiFeedback.improvements.map((improvement, idx) => (
                          <li key={idx} className="flex items-start gap-2">
